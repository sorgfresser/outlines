...
from transformers.models.mixtral.modeling_mixtral import MIXTRAL_INPUTS_DOCSTRING, MoeCausalLMOutputWithPast, \
    _CONFIG_FOR_DOC, add_start_docstrings_to_model_forward, replace_return_docstrings, CrossEntropyLoss, MixtralModel, \
    MoeModelOutputWithPast, DynamicCache, Cache, _prepare_4d_causal_attention_mask, \
    _prepare_4d_causal_attention_mask_for_sdpa, MIXTRAL_ATTENTION_CLASSES, MixtralRMSNorm, \
...
class MixtralSparseMoeBlockRoutable(MixtralSparseMoeBlock):
    ...
    def __init__(self, config):
        ...
        self.combinations_idx = torch.combinations(torch.arange(self.num_experts), self.top_k)

    def forward(self, hidden_states: torch.Tensor,
                experts_used: Optional[List[List[Tuple[int]]]] = None) -> torch.Tensor:
        """ """
        ...
        routing_combs = routing_weights[:, None].expand(-1, self.combinations_idx.shape[0], -1)
        combinations = self.combinations_idx.to(routing_combs.device)[None].expand(router_logits.shape[0], -1, -1)
        combinations_mask = torch.ones(combinations.shape[:-1], dtype=torch.bool,
                                       device=combinations.device)  # (batch * sequence_length, n_combinations)
        ...
        if experts_used is not None:
            for idx, experts_token in enumerate(experts_used):
                ...
                for tok in experts_token:
                    ...
                    combinations_mask[idx] = combinations_mask[idx] & ~((combinations[idx] == expert_comb).all(dim=-1))
        ...
        # Assert sanity
        assert torch.all(combinations_mask[torch.arange(router_logits.shape[0]), selected_idx])
        selected_experts = combinations[torch.arange(router_logits.shape[0]), selected_idx]
...
class MixtralDecoderLayerRoutable(nn.Module):
    ...
    def forward(
        ...
        hidden_states, router_logits, experts = self.block_sparse_moe(hidden_states, experts_used)
        hidden_states = residual + hidden_states
...
class MixtralModelRoutable(MixtralModel):
    def __init__(self, config: "MixtralConfig"):
        ...
        self.layers = nn.ModuleList(
            [MixtralDecoderLayerRoutable(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    use_cache=use_cache,
                    experts_used=experts_used[idx] if experts_used is not None else None,
...
class Mixtral(Transformers):
    def __init__(self, model: "MixtralForCausalLM", tokenizer: "PreTrainedTokenizer", ):
    ...
    def forward(
        ...
        "torch.FloatTensor", Optional[KVCacheType], Optional["torch.FloatTensor"], Optional["torch.FloatTensor"]]:
        ...
        return output.logits, output.past_key_values, output.router_logits, output.experts
    ...
    def __call__(
        ...
        logits, kv_cache, router_logits, experts = self.forward(input_ids, attention_mask, past_key_values,
                                                                experts_used)
        next_token_logits = logits[..., -1, :]

        return next_token_logits, kv_cache, router_logits, experts


def mixtral(
        config: Union["MixtralConfig", "MixtralForCausalLMRoutable"],
        tok_name: str,
    ...
    tok_name
        The name of the tokenizer as listed on Hugging Face's model page.
    device
    ...
    if isinstance(config, MixtralForCausalLMRoutable):
        model = config
    else:
        model = MixtralForCausalLMRoutable(config)
    if device is not None:
        model = model.to(device)

    tokenizer_kwargs.setdefault("padding_side", "left")
    tokenizer = AutoTokenizer.from_pretrained(tok_name, **tokenizer_kwargs)
