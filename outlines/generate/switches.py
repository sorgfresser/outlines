...
def switch_experts_top_k_experts(EXPERTS, EXPERTS_PER_TOK, TOP_K, logits, allowed_tokens, probabilities_gating,
                                 experts_so_far):
    # Only switch experts if we haven't reached the k experts
    if len(experts_so_far) >= TOP_K or len(experts_so_far) >= comb(EXPERTS, EXPERTS_PER_TOK):
        raise TooManyExpertsError("Too many experts")
    print("Switching experts as we haven't reached the top k experts")
    return True


def switch_experts_top_p_experts(EXPERTS, EXPERTS_PER_TOK, TOP_P, logits, allowed_tokens, probabilities_gating,
                                 experts_so_far):
    # Only switch experts if we haven't reached top p experts
    combs = comb(EXPERTS, EXPERTS_PER_TOK)
    combinations = torch.combinations(torch.arange(EXPERTS), EXPERTS_PER_TOK)

    combinations = combinations.to(probabilities_gating.device)
    combinations_mask = torch.zeros(combinations.shape[:-1], dtype=torch.bool,
                                    device=combinations.device)
    probability_combs = probabilities_gating[None, :].expand(combinations.shape[0], -1)
    probability_combs = probability_combs.gather(dim=-1, index=combinations).sum(dim=-1)
    # Normalize the probabilities
    probability_combs = probability_combs / probability_combs.sum()
    for expert in experts_so_far:
        expert_comb = torch.tensor(expert, device=probability_combs.device)
        combinations_mask = combinations_mask | (combinations == expert_comb).all(dim=-1)
    probability_combs = probability_combs.masked_fill(~combinations_mask, 0)
    # Sum
    final_value = probability_combs.sum(dim=0)

    if final_value.item() >= TOP_P or len(experts_so_far) >= comb(EXPERTS, EXPERTS_PER_TOK):
        raise TooManyExpertsError("Too many experts")
